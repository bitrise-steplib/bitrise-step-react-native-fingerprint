...
and folders in `path_list` (folders hashed recursively), minus anything matched by
`ignore_path_list`. That is fast and dependency-light, but it is *syntactic*: it
...
- **Native changes that live only in `node_modules`.** A native module pulled
  ...
  without a lockfile change, is not hashed unless you add it to `path_list`.
...
- **Misconfiguration.** Removing `ios`/`android` from `path_list`, a wrong
  `project_dir`, or an `ignore_path_list` entry that is too broad can silently drop
  a real input.

## Why `ignore_path_list` matters (correctness, not just speed)
...
- Extend `path_list` to cover native inputs outside the standard `ios`/`android`
  layout; tighten `ignore_path_list` if a volatile file is sneaking into the hash.
