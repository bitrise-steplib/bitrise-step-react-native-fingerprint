...
| `key` | Advanced escape hatch. When non-empty, the `paths` / `ignore_paths` / `key_prefix` mechanism is bypassed and this template is evaluated as the key, using the same engine as the `restore-cache` / `save-cache` steps: - `{{ checksum "package.json" "android/**/*.gradle" }}` — SHA-256 over files (glob / `**` supported). - `{{ getenv "SOME_ENV" }}` — interpolate an environment variable. - `.OS`, `.Arch`, `.Branch`, `.CommitHash`, `.Workflow` — build metadata.  Leave empty to use the default folder-based fingerprint.  |  |  |
